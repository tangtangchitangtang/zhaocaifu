...
- (void)setImgArray:(NSArray *)imgArray{
    ...
    [self addImgClick];
    if(_imgArray.count<2) //只有一张图片就不用轮播了
    {
        [self startTimer];
    }
...
- (UIPageControl *)pageControl{
    if (!_pageControl) {
        CGRect rect = CGRectMake(0, self.height-_fenYeheight, self.width, 4);
        _pageControl = [[UIPageControl alloc] initWithFrame:rect];
        self.pageControl.currentPage = 0;
        self.pageControl.currentPageIndicatorTintColor = _backgroundColor;
        self.pageControl.pageIndicatorTintColor = ColorWithRGB(220, 220, 220);
    }
    return _pageControl;
}
